...
var Board = function (size) {
    ...
    this.getSquare = function() {
        ...
        for (var r = 0, i = this.square.length; r < i; r++) {
            returnSquare.push([]);
            for (var c = 0, j = this.square[r].length; c < j; c++) {
                returnSquare[r][c] = this.square[r][c];
            }
        }
        return returnSquare;
    };
    ...
    this.promote = function (checker) {
        checker.isKing = true;
        this.dispatchBoardEvent("promote", {
            checker: checker
        });
    }
